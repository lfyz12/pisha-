...
Фронтенд — Vite SPA, деплоится из директории `frontend/` (конфиг — `frontend/vercel.json`):

1. Импортируйте GitHub-репозиторий в Vercel и укажите **Root Directory: `frontend`**. Framework определится как Vite, сборка — `npm run build` → `dist`.
2. В `frontend/vercel.json` замените `YOUR-NGROK-DOMAIN` на публичный домен бэкенда (например, домен ngrok-туннеля, см. `docker-compose.ngrok.yml`). Все запросы `/api/*` Vercel проксирует туда — браузер работает только с доменом Vercel, поэтому CORS и SameSite-куки не мешают.
3. Домен бэкенда должен быть в `ALLOWED_HOSTS` на стороне Django.
4. ⚠️ Basic Auth ngrok (`NGROK_BASIC_AUTH`) несовместим с проксированием из Vercel — запросы Vercel не знают пароль. Для связки с Vercel оставьте туннель без Basic Auth (приложение и так защищено логином + TOTP).
5. SSE-эндпоинты ИИ-чата (`/api/ai/`) через Vercel-прокси могут буферизоваться — для полноценного чата используйте прямой доступ по ngrok-ссылке.

На бесплатном плане ngrok домен меняется при перезапуске туннеля — обновляйте `destination` в `frontend/vercel.json` и коммитьте (или заведите reserved domain).
